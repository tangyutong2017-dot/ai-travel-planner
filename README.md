...
| **关键成果** | 编造地名实测 9/9 被拦下、真实地名 11/11 通过;62 项接口断言;约 7,900 行代码 + 6 份决策文档,每个「不做」的决定都写了原因 |
...
- **结构化表单** —— 出发城市、目的地、日期、同行关系、初访/重游、城际交通、市内交通、兴趣、节奏、舒适度、体力接受度，外加一段自由描述交给 LLM 解析
- **五步进度反馈** —— 后台任务 + 前端轮询，每一步说明当前在做什么（生成要 1~2 分钟，进度条不能空转）
- **联网查证** —— 开放时间、是否需预约这类时效信息走 Tavily 搜索，不靠模型记忆
- **逐点高德核实** —— 坐标、地址、实拍图、门票价格；查不到的条目标为「未核实」而不是编一个
- **真实驾车通勤** —— 段间耗时来自高德路线接口，不由模型估算
- **天气** —— 按每天的实际日期匹配预报；超出 4 天预报范围时如实显示「暂无预报」
- **住宿片区** —— 只给片区不推荐具体酒店（无法核实空房与价格）

![创建向导](docs/images/01-创建向导.png)

### 工作区

- **逐日时间线** —— 景点卡片含类型、时段、时长、费用、是否需预约、推荐理由
- **当日动线地图** —— 高德静态图，key 留在服务端由后端代理
- **手动编辑** —— 改标题/时段/时长/类型/费用/说明，删除条目；不依赖 AI，模型不可用时仍能改
- **侧栏信息** —— 天气、住宿片区、数据质量统计（多少条有坐标/图片/POI）、行程说明（当日安排理由 + 全程提示）
- **三档响应式** —— <1280px 收右栏，<1024px 收日程栏改横向条

![行程工作区](docs/images/03-工作区.png)

> 左为日程导航，中为逐日时间线，右为地图、天气、住宿与行程说明；底部是 AI 对话编辑栏。

### AI 对话编辑

- **自然语言增删改** —— 「第二天太赶了，删掉一个景点」「中午加个午饭」「把午饭改成 90 分钟」
- **先搜后选** —— 要加地点时模型先查高德拿到真实候选，再从中挑选，而不是凭记忆写店名
- **代码生成回执** —— 绿色 ✓ 是实际执行成功的操作，模型的话单独显示，两者不混淆
- **改动高亮** —— 被改的条目在时间线上加绿框，十几项里一眼找到变化
- **撤销** —— 最近 10 次改动可逐级回退；AI 与手动改动都进快照

### 输出

- **杂志排版行程册** —— 封面总览页（满幅封面图 + 行程速览 + 出行提示）+ 每日单页（大号日期 + 半幅动线地图 + 逐条安排 + 当晚住宿）
- **导出 PDF** —— 浏览器直接打印，纯前端渲染库中数据，不调 LLM、不消耗额度

| 封面总览页 | 每日单页 |
|---|---|
| ![行程册封面](docs/images/04-行程册封面.png) | ![行程册每日页](docs/images/05-行程册每日页.webp) |

### 我的行程

- 列表、按状态筛选、关键词搜索、三种排序、重命名、删除、自动封面图

![我的行程](docs/images/02-我的行程.png)

### 工程

- 62 项接口冒烟断言覆盖全部路由与错误分支
- GitHub Actions CI：前端格式/构建/lint + 后端编译与导入检查
- Langfuse 接入 LLM 调用链路，可观测每次生成的耗时与 token
...
| [`智能旅行规划助手-PRD-v0.1.md`](./docs/智能旅行规划助手-PRD-v0.1.md) | 需求文档；第 14 章逐条标注了每项需求最后做了没有、为什么 |
| [`智能旅行规划助手-产品立项文档-v0.1.md`](./docs/智能旅行规划助手-产品立项文档-v0.1.md) | 立项期的定位与范围推演 |
| [`智能旅行规划助手-Figma低保真设计说明-v0.1.md`](./docs/智能旅行规划助手-Figma低保真设计说明-v0.1.md) | 低保真设计阶段的页面结构与交互定义 |
